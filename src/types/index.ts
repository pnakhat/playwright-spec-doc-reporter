...
export interface CucumberConfig {
  /**
   * Path(s) to Cucumber JSON report files generated by @cucumber/cucumber
   * JSON formatter (--format json:cucumber-report.json).
   *
   * These results are merged into the glossy reporter output alongside any
   * Playwright test results.
   *
   * Example: "cucumber-report.json"
   * Example: ["reports/cucumber.json", "reports/cucumber-2.json"]
   */
  jsonReports?: string | string[];

  /**
   * When true, the reporter will automatically detect playwright-bdd tests
   * (scenarios generated from .feature files via playwright-bdd) and enhance
   * them with Cucumber-specific metadata extraction.
   *
   * Default: true
   */
...
export interface GlossyReporterConfig {
  ...
  /**
   * Cucumber integration configuration.
   *
   * Enables two integration modes:
   * 1. **Cucumber JSON ingestion** — parse @cucumber/cucumber JSON reports and
   *    merge them into the glossy report alongside Playwright results.
   * 2. **playwright-bdd enhancement** — detect playwright-bdd generated tests
   *    and enrich them with BDD metadata (feature, scenario, Gherkin steps).
   *
   * Example (JS/TS):
   * ```ts
   * cucumber: {
