...
import { commitPatchedFiles } from "../autofix/branchManager.js";
...
async function main() {
  ...
  for (let i = 0; i < args.length; i++) {
    if (args[i] === "--report" && args[i + 1]) {
      ...
      ++i; // consumed — platform auto-detected from env vars
  ...
  if (fixed > 0) {
    if (createPR) {
      log(`\n${c.cyan}🚀 Creating auto-fix PR (--create-pr)...${c.reset}`);

      // The healing agent already wrote fixes to disk via Claude.
      // Just commit the patched files and open a draft PR.
      const patchedFiles = results.filter(r => r.status === "fixed").map(r => r.file);

      try {
        const branchResult = await commitPatchedFiles({
          patchedFiles,
          cwd: process.cwd(),
          context: patchedFiles[0],
        });
        ...
        if (!token || !repo) {
          log(`  ${c.yellow}⚠ Branch pushed: ${branchResult.branchName}${c.reset}`);
          log(`  ${c.gray}Set GITHUB_TOKEN + GITHUB_REPOSITORY to auto-create the PR${c.reset}\n`);
        } else {
          const title = `fix(autofix): heal ${patchedFiles.length} failing test(s) — AI-suggested patches`;
          const body = [
            `## 🩹 Playwright Auto-Fix`,
            ``,
            `Branch: \`${branchResult.branchName}\` | Commit: \`${branchResult.commitSha}\``,
            ``,
            `### 📁 Patched files`,
            patchedFiles.map(f => `- \`${f}\``).join("\n"),
            ``,
            `### ✅ Test plan`,
            `- [ ] Review each patched file`,
            `- [ ] Run \`npx playwright test\``,
            `- [ ] Approve and merge once green`,
            ``,
            `🤖 Generated by Playwright Glossy Reporter — healing agent`,
          ].join("\n");
          ...
          if (createRes.ok) {
            ...
            await fetch(`https://api.github.com/repos/${repo}/issues/${pr.number}/labels`, {
              ...
              body: JSON.stringify({ labels: ["auto-fix", "test-failure", "ai-generated"] }),
            }).catch(() => { /* best-effort */ });

            log(`  ${c.green}✓ Draft PR created → ${pr.html_url}${c.reset}`);
            log(`  ${c.gray}Branch: ${branchResult.branchName} | Commit: ${branchResult.commitSha}${c.reset}\n`);
          } else {
            const err = await createRes.text().catch(() => "");
            log(`  ${c.red}✗ PR API error (${createRes.status}): ${err.slice(0, 200)}${c.reset}\n`);
          }
        }
      } catch (err) {
        log(`  ${c.red}✗ PR creation failed: ${err instanceof Error ? err.message : String(err)}${c.reset}\n`);
      }
    } else {
      ...
      log(`  4. Or rerun with ${c.white}--create-pr${c.gray} to open a draft PR automatically\n`);
    }
  }
}
